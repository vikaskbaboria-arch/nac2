export const fetchgenere=async(genre)=>{
    console.log(genre)
    const data = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${process.env.NEXT_PUBLIC_API_KEY}&with_genres=${genre}`,{ next: { revalidate: 3600 } })
...
export const generename=async()=>{
  ...
  `https://api.themoviedb.org/3/genre/movie/list?api_key=${process.env.NEXT_PUBLIC_API_KEY}`,{ next: { revalidate: 3600 } }
);
