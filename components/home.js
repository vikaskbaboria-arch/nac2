              ...
              {extended.map((m, idx) => (
                ...
                className="flex-shrink-0 p-2" style={{ width: `${slideWidth}px` }}>
                  <div className="h-[20vh] md:h-[50vh] lg:h-[60vh] relative rounded-lg overflow-hidden border-2 border-purple-950">
                    ...
                    <div className="absolute top-2 sm:top-4  w-14 h-4 right-2 sm:right-4 text-xs bg-green-700 text-white px-2 rounded flex items-center justify-center"> ⭐{m?.vote_average}</div>
                  </div>
                </div>
              ))}
            </div>
